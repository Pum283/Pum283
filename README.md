<img src="https://i.pinimg.com/originals/bc/64/ec/bc64eca2ac244d7886d5da39ebcf13a7.gif" alt="Coding Banner" width="100%"/>

# Chào mọi người, I'm Nguyễn Đình Mạnh Hùng - Pum 👋

🖥️ **.NET Developer** từ Việt Nam  
💼 Chuyên C#, ASP.NET Core, Entity Framework – Đam mê xây dựng backend vững chắc, API hiệu suất cao và ứng dụng enterprise.  
🌱 Hiện đang học sâu hơn về .NET 8/9, Blazor, Azure và Microservices.  
🔭 Đang phát triển các dự án cá nhân với .NET để hoàn thiện portfolio.  
👯 Mở collab trên các dự án .NET, API hoặc fullstack.  
📫 Liên hệ mình qua: 
- Email: [manhhung02082003@gmail.com](mailto:your-email@example.com)

# Hi everyone, I'm Nguyễn Đình Mạnh Hùng (Pum) 👋

🖥️ **.NET Developer** from Vietnam
💼 Specialized in C#, ASP.NET Core, and Entity Framework – passionate about building robust backends, high-performance APIs, and enterprise applications.
🌱 Currently diving deeper into .NET 8/9, Blazor, Azure, and Microservices architecture.
🔭 Working on personal .NET projects to strengthen my portfolio.
👯 Open to collaboration on .NET, API, or fullstack projects.
📫 Contact me:

- Email: [manhhung02082003@gmail.com](mailto:your-email@example.com)
---------------------------------------------------------------------------------------------
# Chào mọi người, mình là Nguyễn Đình Mạnh Hùng (Pum) 👋

🖥️ **.NET Developer** đến từ Việt Nam
💼 Chuyên về C#, ASP.NET Core, Entity Framework – tập trung xây dựng backend vững chắc, API hiệu suất cao và các ứng dụng doanh nghiệp.
🌱 Hiện đang đào sâu kiến thức về .NET 8/9, Blazor, Azure và kiến trúc Microservices.
🔭 Đang phát triển các dự án cá nhân với .NET để hoàn thiện portfolio.
👯 Sẵn sàng hợp tác trong các dự án .NET, API hoặc fullstack.
📫 Liên hệ:

- Email: [manhhung02082003@gmail.com](mailto:your-email@example.com)
<!-- LinkedIn: [linkedin.com/in/your-profile](https://linkedin.com/in/your-profile) -->

### 🛠️ Tech Stack & Tools
<img src="https://skillicons.dev/icons?i=dotnet,csharp,aspnet,visualstudio,vscode,git,github,azure,sqlserver,postgres,mongodb" />
<!-- Thêm/bớt icon tại https://skillicons.dev (có sẵn dotnet, csharp, aspnet, azure, sqlserver...) -->

### 📊 GitHub Stats

⚡ **Fun fact**: Sáng mà làm ly đen đá không đường thì hết bài ☕ 🇻🇳
